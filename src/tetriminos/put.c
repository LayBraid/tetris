...
void put_tetriminos(tetris_t *tetris)
{
    for (int i = 0; i < tetris->next_tetriminos->row; i++) {
        for (int j = 0; j < tetris->next_tetriminos->col; j++) {
            ...
            tetris->block[tetris->next_tetriminos->y + i + 1][tetris->next_tetriminos->x + j] =
            (tetris->next_tetriminos->design[i][j] != 0 ?
            tetris->next_tetriminos->design[i][j] : tetris->block[tetris->next_tetriminos->y + i + 1][tetris->next_tetriminos->x + j]);
        }
    }
    tetris->opt->level++;
